...
namespace RiotControl
{
	partial class WebService
	{
		List<GameTeamPlayer> LoadSummonerGames(Summoner summoner, NpgsqlConnection database)
		{
			...
			DatabaseCommand select = GetCommand("select {0} from game_result, team_player where game_result.id = team_player.game_id and team_player.summoner_id = :summoner_id order by game_result.game_time desc", database, GameTeamPlayer.GetFields());
