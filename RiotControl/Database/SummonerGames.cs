...
namespace RiotControl
{
	partial class WebService
	{
		List<GameTeamPlayer> LoadSummonerGames(Summoner summoner, DbConnection database)
		{
			...
			DatabaseCommand select = GetCommand("select {0} from game, player where game.id = player.game_id and player.summoner_id = :summoner_id order by game.time desc", database, GameTeamPlayer.GetFields());
